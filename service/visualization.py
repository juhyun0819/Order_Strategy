...
def create_client_sales_chart(df, selected_product, client_count, all_dates=None):
    if not selected_product or client_count is None:
        return None
    df = df.copy()
    df['판매일자'] = pd.to_datetime(df['판매일자'])
    current_year = datetime.datetime.now().year
    df_current_year = df[df['판매일자'].dt.year == current_year]
    product_df = df_current_year[df_current_year['품명'] == selected_product]
    if product_df.empty:
        return None
    if all_dates is not None and len(all_dates) > 0:
        idx = pd.to_datetime(all_dates)
        daily_sales = product_df.groupby('판매일자')['실판매'].sum()
        daily_sales = daily_sales.reindex(idx, fill_value=0).reset_index()
        daily_sales.columns = ['판매일자', '실판매']
    else:
        daily_sales = product_df.groupby('판매일자')['실판매'].sum().reset_index()
    # 날짜별 데이터
    date_strs = daily_sales['판매일자'].dt.strftime('%Y-%m-%d')
    client_data = [[d, client_count] for d in date_strs]
    sales_data = [[d, v] for d, v in zip(date_strs, daily_sales['실판매'])]
    chart_config = {
        'title': {'text': f'{selected_product} - 거래처 수 & 판매량', 'left': 'center'},
        'tooltip': {
            'trigger': 'axis',
            'axisPointer': {'animation': False},
        },
        'legend': {'data': ['거래처 수', '판매량']},
        'xAxis': {'type': 'time', 'splitLine': {'show': False}},
        'yAxis': [
            {'type': 'value', 'name': '거래처 수', 'position': 'left'},
            {'type': 'value', 'name': '판매량', 'position': 'right'}
        ],
        'series': [
            {
                'name': '거래처 수',
    ...
    return {
        'type': 'combined',
        'title': f'{selected_product} - 거래처 수 & 판매량',
        'config': chart_config,
        'data': {
...
def create_visualizations(df, only_product=False, all_dates=None, trend_window=7, trend_frac=0.08, client_count=0, product=None):
    """ECharts용 대시보드 그래프 데이터 생성"""
    from service.db import get_client_history
    import pandas as pd
    charts = {}
    df = df.copy()
    df['판매일자'] = pd.to_datetime(df['판매일자'])
    ...
    if product:
        ...
        # 2. 데이터프레임을 all_dates로 reindex (없는 날짜는 0)
        daily_sales = df.groupby('판매일자')['실판매'].sum()
        daily_sales = daily_sales.reindex(all_dates, fill_value=0).reset_index()
        daily_sales.columns = ['판매일자', '실판매']
        date_strs = daily_sales['판매일자'].dt.strftime('%Y-%m-%d')
        # 3. 거래처 수 이력 기반 시계열 생성 (오늘 이후는 0)
        client_data = []
        sales_data = []
        history = get_client_history(product)
        hist_idx = 0
        cur_val = 0
        for d, v in zip(daily_sales['판매일자'], daily_sales['실판매']):
            d_str = d.strftime('%Y-%m-%d')
            if d.date() > today:
                client_data.append([d_str, 0])
                sales_data.append([d_str, None])
                continue
            while hist_idx < len(history) and d_str >= history[hist_idx][0]:
                cur_val = history[hist_idx][1]
                hist_idx += 1
            client_data.append([d_str, cur_val])
            sales_data.append([d_str, v if v != 0 else None])
        
        charts['sales_trend'] = {
            'type': 'line',
            'title': f'{current_year}년 판매 트렌드',
            'data': {
                'client_data': client_data,
                'sales_data': sales_data
            },
            'config': {
                'title': {'text': f'{current_year}년 판매 추세', 'left': 'center'},
                'tooltip': {
                    'trigger': 'axis',
                    'axisPointer': {'animation': False},
                },
        ...
        if not df.empty:
            ...
            if not lastyear_df.empty:
                ...
                forecast_dates = pd.date_range(start=lastyear_daily['판매일자'].iloc[-1] + datetime.timedelta(days=1), periods=7)
